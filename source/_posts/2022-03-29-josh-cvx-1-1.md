...
> **举例**&emsp;**线性方程组的解集**。线性方程组的解集 $C = \{x\ |\ Ax= b\}$，其中 $A\in\bfR^{m\times n}$，$b \in \bfR^{m}$ 是一个仿射集合。为说明这点，设 $x_1,x_2\in C$，即 $Ax_1 = b, Ax_2 = b$。则对于任意 $\theta$，有
>
> $$\begin{aligned}
...
> **举例**&emsp;考虑 $\bfR^2$ 上的单位圆环 $\left\{x\in\bfR^2\ \middle|\ x_1^2 + x_2^2 = 1 \right\}$。它的仿射包是全空间 $\bfR^2$，所以其仿射维数为 $2$。但是，在其他大多数维数的定义下，$\bfR^2$ 上的单位圆环的维数为 $1$。
...
>
> $$
> C = \{x \in \bfR \ |\ -1\leqslant 1, -1\leqslant x_2\leqslant 1, x_3 = 0\}
> $$
>
> 其仿射包为 $(x_1,x_2)$-平面，即 $\aff C = \{x\in\bfR^3\ |\ x_3 = 0 \}$。$C$ 的内部为空，但其相对内部为
>
> $$
> \relint C = \{x\in\bfR^3\ |\  -1<x_1<1,-1<x_2<1,x_3 = 0\}
> $$
>
> $C$（在 $\bfR^3$ 中）的边界是其自身，而相对边界是其边框，
>
> $$
> \{x \in \bfR^3\ |\ \max\{|x_1|,|x_2|\} = 1,x_3 = 0\}
> $$
...
![图 4. 扇形展示了所有具有形式 $\theta_1 x_1 + \theta_2 x_2$ 的点，其中 $\theta_1, \theta_2 \geqslant 0$。扇形的顶点 ($\theta_1 = \theta_2 = 0$) 在 $0$；其边界（对应于 $$\theta_1 = 0$ 或 $\theta_2 = 0$) 穿过点 $x_1$ 和 $x_2$。](../images/post/2022-03-29-josh-cvx-1-1/2022-03-29-josh-cvx-1-1-040-ThePieSlice.png){width=400px}
...
![图 5. [图 3](#图3) 中两个集合的锥包（阴影所示）。](../images/post/2022-03-29-josh-cvx-1-1/2022-03-29-josh-cvx-1-1-050-TheConicHulls.png){width=600px}
